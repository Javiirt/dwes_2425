...
class Alumno extends Controller
{
    ...
    */
    public function render()
    {
        ...
        // Compruebo si hay un mensaje en la sesión
    ...
    */
    public function nuevo()
    {
        ...
        // Creo un objeto de la clase alumno vacío
    ...
    */
    public function create()
    {
        ...
        // Sirve para prevenir ataques XSS
        ...
        // Para email hay un filter especial
        ...
        if (empty($fechaNac)) {
            ...
            if (!$fecha) {
                $error['fechaNac'] = "La fecha de nacimiento no tiene el formato correcto";
            }
        }

        // Validación de DNI
        // Reglas: obligatorio, formato y clave secundaria

        //Expresion regular para validar el dni
        // /^[0-9]{8}[A-Z]$/

        $option = ['options' => ['regexp' => '/^[0-9]{8}[A-Z]$/']];

        if (empty($dni)) {
            $error['dni'] = "El DNI es obligatorio";
        } else if (!filter_var($dni, FILTER_VALIDATE_REGEXP, $option)) {
            $error['dni'] = "El DNI no tiene el formato correcto";
        } else if (!$this->model->validateUniqueDNI($dni)) {
            $error['dni'] = "El DNI no tiene la clave correcta";
        }

        // Validación de email
        // Reglas: obligatorio, formato
        if (empty($email)) {
            $error['email'] = "El email es obligatorio";
        } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error['email'] = "El email no tiene el formato correcto";
        ...
        if (empty($telefono)) {
            ...
            $error['telefono'] = "El teléfono no tiene el formato correcto";
        }

        // Validación de nacionalidad
        // Reglas: No obligatorio

        // Validación de id_curso
        ...
        if (empty($id_curso)) {
            ...
            $error['id_curso'] = "El curso no tiene el formato correcto";
        } elseif (!$this->model->validateForeignKey($id_curso)) {
        ...
        if (!empty($error)) {
            ...
            //Creo la variable de sesión alumno
        ...
        $_SESSION['mensaje'] = "Alumno añadido correctamente";
